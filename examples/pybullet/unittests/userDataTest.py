...
class TestUserDataMethods(unittest.TestCase):
  ...
  def testMultipleLinks(self):
    ...
    for link_index in range(num_links):
      uid1 = self.client.addUserData(
          body_id, "MyKey1", "Value1 for link %s" % link_index, link_index)
      uid2 = self.client.addUserData(
          body_id, "MyKey2", "Value2 for link %s" % link_index, link_index)

    for link_index in range(num_links):
      uid1 = self.client.getUserDataId(body_id, "MyKey1", link_index)
      uid2 = self.client.getUserDataId(body_id, "MyKey2", link_index)
      self.assertEqual(("Value1 for link %s" % link_index).encode(),
                       self.client.getUserData(uid1))
      self.assertEqual(("Value2 for link %s" % link_index).encode(),
                       self.client.getUserData(uid2))
  ...
  def testUserDataOnVisualShapes(self):
    ...
    for link_index in range(-1, num_links):
      ...
      for shape_index in range(num_shapes):
        ...
        uid = self.client.addUserData(body_id, key, value, link_index,
                                      shape_index)
        user_data_entries.add((uid, key, value.encode(), body_id, link_index,
                               shape_index))
    ...
    for uid, key, value, body_id, link_index, shape_index in user_data_entries:
      ...
      self.assertEqual(
          uid, self.client.getUserDataId(body_id, key, link_index, shape_index))
