...
id_simulator = bullet.connect(
    bullet.GUI)  # or bullet.DIRECT for non-graphical version
...
bullet.setJointMotorControlArray(
    id_robot, id_revolute_joints, bullet.VELOCITY_CONTROL, forces=[0.0, 0.0])
...
for s in range(steps):
  t[s] = start + s * delta_t
  q_pos_desired[0][s] = 1. / (2. * math.pi) * math.sin(
      2. * math.pi * t[s]) - t[s]
  q_pos_desired[1][s] = -1. / (2. * math.pi) * (
      math.cos(2. * math.pi * t[s]) - 1.0)
...
if plot:
  ...
  figure.subplots_adjust(
      left=0.05, bottom=0.11, right=0.97, top=0.9, wspace=0.4, hspace=0.55)
