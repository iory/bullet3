...
p.connect(p.GUI)
planeId = p.loadURDF(
    fileName="plane.urdf", baseOrientation=[0.25882, 0, 0, 0.96593])
p.loadURDF(fileName="cube.urdf", basePosition=[0, 0, 2])
cubeId = p.loadURDF(
    fileName="cube.urdf", baseOrientation=[0, 0, 0, 1], basePosition=[0, 0, 4])
...
def drawInertiaBox(parentUid, parentLinkIndex):
  mass, frictionCoeff, inertia = p.getDynamicsInfo(
      bodyUniqueId=parentUid,
