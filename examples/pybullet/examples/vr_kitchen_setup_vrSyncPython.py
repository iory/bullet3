...
if (once):
  # p.startStateLogging(p.STATE_LOGGING_PROFILE_TIMINGS, "userDebugItems1.json")
  logId = -1
        ...
        if (hitObjectUid >= 0):
          objectInfo = str(hitObjectUid) + " Link Index=" + str(
              linkIndex) + "\nBase Name:" + p.getBodyInfo(
                  hitObjectUid)[0].decode() + "\nBody Info:" + p.getBodyInfo(
                      hitObjectUid)[1].decode()
        else:
          objectInfo = "None"
