...
for i in range(int(num * 50)):
  ...
  orn = p.getQuaternionFromEuler(
      [0, 0, 0.5 * math.pi + math.pi * 2 * i / float(num)])
  pos = [
...
sphere = p.loadURDF(
    "domino/domino.urdf", pos, orn, useMaximalCoordinates=useMaximalCoordinates)
...
p.changeDynamics(
    door, 1, linearDamping=0, angularDamping=0, jointDamping=0, mass=1)
...
while (1):
  ...
  if position_control:
    ...
    p.setJointMotorControl2(
        door, 1, p.VELOCITY_CONTROL, targetVelocity=1, force=1011)
