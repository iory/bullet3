...
class Policy():
  ...
  def update(self, rollouts, sigma_r, args):
    ...
    np.save(args.logdir + "/policy_" + self.env_name + "_" + timestr + ".npy",
            self.theta)
  ...
  for step in range(hp.nb_steps):
    ...
    if parentPipes:
      for k in range(hp.nb_directions):
        ...
        parentPipe.send(
            [_EXPLORE, [normalizer, policy, hp, "positive", deltas[k]]])
      for k in range(hp.nb_directions):
      ...
      for k in range(hp.nb_directions):
        parentPipe = parentPipes[k]
        parentPipe.send(
            [_EXPLORE, [normalizer, policy, hp, "negative", deltas[k]]])
      for k in range(hp.nb_directions):
      ...
      for k in range(hp.nb_directions):
        positive_rewards[k] = explore(env, normalizer, policy, "positive",
                                      deltas[k], hp)

      # Getting the negative rewards in the negative/opposite directions
      for k in range(hp.nb_directions):
        negative_rewards[k] = explore(env, normalizer, policy, "negative",
                                      deltas[k], hp)
    ...
    order = sorted(
        scores.keys(), key=lambda x: scores[x])[:hp.nb_best_directions]
    rollouts = [
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      '--movie', help='rgb_array gym movie', type=int, default=0)
  parser.add_argument(
      '--steps', help='Number of steps', type=int, default=10000)
  parser.add_argument(
      '--policy', help='Starting policy file (npy)', type=str, default='')
  parser.add_argument(
      '--logdir',
      ...
      default='.')
  parser.add_argument(
      '--mp', help='Enable multiprocessing', type=int, default=1)
