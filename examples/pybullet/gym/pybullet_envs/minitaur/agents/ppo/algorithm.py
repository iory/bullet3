...
_NetworkOutput = collections.namedtuple('NetworkOutput',
                                        'policy, mean, logstd, value, state')
...
class PPOAlgorithm(object):
  ...
  def perform(self, observ):
    ...
    with tf.name_scope('perform/'):
      ...
      action = tf.cond(self._is_training,
                       network.policy.sample, lambda: network.mean)
  ...
  def experience(self, observ, action, reward, unused_done, unused_nextob):
    ...
    with tf.name_scope('experience/'):
      return tf.cond(self._is_training, lambda: self._define_experience(
          observ, action, reward), str)
  ...
  def _training(self):
    ...
    with tf.name_scope('training'):
      ...
      policy_summary = self._update_policy(observ, action, old_mean, old_logstd,
                                           reward, length)
  ...
  def _update_policy_step(self, observ, action, old_mean, old_logstd, advantage,
                          length):
    ...
    loss, summary = self._policy_loss(network.mean, network.logstd, old_mean,
