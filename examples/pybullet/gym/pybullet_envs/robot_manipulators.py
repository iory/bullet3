...
class Reacher(MJCFBasedRobot):
  ...
  def __init__(self):
    MJCFBasedRobot.__init__(
        self, 'reacher.xml', 'body0', action_dim=2, obs_dim=9)
...
class Pusher(MJCFBasedRobot):
  ...
  def __init__(self):
    MJCFBasedRobot.__init__(
        self, 'pusher.xml', 'body0', action_dim=7, obs_dim=55)
  ...
  def apply_action(self, a):
    assert (np.isfinite(a).all())
    self.shoulder_pan_joint.set_motor_torque(
        0.05 * float(np.clip(a[0], -1, +1)))
    self.shoulder_lift_joint.set_motor_torque(
        0.05 * float(np.clip(a[1], -1, +1)))
    self.upper_arm_roll_joint.set_motor_torque(
        0.05 * float(np.clip(a[2], -1, +1)))
    self.elbow_flex_joint.set_motor_torque(0.05 * float(np.clip(a[3], -1, +1)))
    self.forearm_roll_joint.set_motor_torque(
        0.05 * float(np.clip(a[4], -1, +1)))
  ...
  def calc_state(self):
    ...
    return np.concatenate([
        np.array([j.current_position() for j in self.ordered_joints
...
class Striker(MJCFBasedRobot):
  ...
  def __init__(self):
    MJCFBasedRobot.__init__(
        self, 'striker.xml', 'body0', action_dim=7, obs_dim=55)
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    self.target.reset_pose(self.target_pos - self.zero_offset,
                           np.array([0, 0, 0, 1]))
...
class Thrower(MJCFBasedRobot):
  ...
  def __init__(self):
    MJCFBasedRobot.__init__(
        self, 'thrower.xml', 'body0', action_dim=7, obs_dim=48)
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    # reset object position
    self.parts["object"].reset_pose(self.object_pos - self.zero_offset,
                                    np.array([0, 0, 0, 1]))
    ...
    self.parts["target"].reset_pose(self.target_pos - self.zero_offset,
                                    np.array([0, 0, 0, 1]))
