...
class ReacherBulletEnv(MJCFBaseBulletEnv):
  ...
  def step(self, a):
    ...
    stuck_joint_cost = -0.1 if np.abs(np.abs(self.robot.gamma) -
                                      1) < 0.01 else 0.0
    self.rewards = [
...
class PusherBulletEnv(MJCFBaseBulletEnv):
  ...
  def create_single_player_scene(self, bullet_client):
    return SingleRobotEmptyScene(
        bullet_client, gravity=9.81, timestep=0.0020, frame_skip=5)
...
class StrikerBulletEnv(MJCFBaseBulletEnv):
  ...
  def step(self, a):
    ...
    dist_object_finger = self.robot.object.pose().xyz(
    ) - self.robot.fingertip.pose().xyz()
    reward_dist_vec = self.robot.object.pose().xyz() - self.robot.target.pose(
    ).xyz(
    )  # TODO: Should the object and target really belong to the robot? Maybe split this off

    self._min_strike_dist = min(self._min_strike_dist,
                                np.linalg.norm(reward_dist_vec))
    ...
    if self._striked:
      ...
      reward_near_vec = self.robot.object.pose().xyz(
      ) - self.robot.fingertip.pose().xyz()
    ...
    self.rewards = [
        float(self.potential - potential_old),
        float(electricity_cost),
        float(stuck_joint_cost), 3 * reward_dist, 0.1 * reward_ctrl,
        0.5 * reward_near
    ]
    self.HUD(state, a, False)
    return state, sum(self.rewards), False, {}
...
class ThrowerBulletEnv(MJCFBaseBulletEnv):
  ...
  def create_single_player_scene(self, bullet_client):
    return SingleRobotEmptyScene(
        bullet_client, gravity=0.0, timestep=0.0020, frame_skip=5)
