...
class XmlBasedRobot:
  ...
  def addToScene(self, bullet_client, bodies):
    ...
    if np.isscalar(
        bodies):  # streamline the case where bodies is actually just one body
      bodies = [bodies]
    ...
    for i in range(len(bodies)):
      ...
      for j in range(self._p.getNumJoints(bodies[i])):
        ...
        if i == 0 and j == 0 and self.robot_body is None:  # if nothing else works, we take this as robot_body
          parts[self.robot_name] = BodyPart(self._p, self.robot_name, bodies, 0,
                                            -1)
...
class MJCFBasedRobot(XmlBasedRobot):
  ...
  def __init__(self,
               ...
               obs_dim,
               self_collision=True):
    XmlBasedRobot.__init__(self, robot_name, action_dim, obs_dim,
                           self_collision)
    self.model_xml = model_xml
    self.doneLoading = 0
...
class BodyPart:
  ...
  def speed(self):
    if self.bodyPartIndex == -1:
      ...
      (x, y, z), (a, b, c,
                  d), _, _, _, _, (vx, vy,
                                   vz), (vr, vp, vy) = self._p.getLinkState(
