...
class HalfCheetah(WalkerBase):
  ...
  def __init__(self):
    WalkerBase.__init__(
        self, "half_cheetah.xml", "torso", action_dim=6, obs_dim=26, power=0.90)

  def alive_bonus(self, z, pitch):
    # Use contact other than feet to terminate episode: due to a lot of strange walks using knees
    return +1 if np.abs(
        pitch) < 1.0 and not self.feet_contact[1] and not self.feet_contact[
...
class Ant(WalkerBase):
  ...
  def __init__(self):
    WalkerBase.__init__(
        self, "ant.xml", "torso", action_dim=8, obs_dim=28, power=2.5)
  ...
  def flag_reposition(self):
    ...
    if (self.flag):
      ...
      self._p.resetBasePositionAndOrientation(
          self.flag.bodies[0], [self.walk_target_x, self.walk_target_y, 0.7],
          [0, 0, 0, 1])
    else:
      self.flag = get_sphere(self._p, self.walk_target_x, self.walk_target_y,
                             0.7)
  ...
  def calc_state(self):
    ...
    if self.walk_target_dist < 1 or self.flag_timeout <= 0:
      ...
      state = Humanoid.calc_state(
          self)  # caclulate state again, against new flag pos
...
class HumanoidFlagrunHarder(HumanoidFlagrun):
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    if (self.aggressive_cube):
      self._p.resetBasePositionAndOrientation(self.aggressive_cube.bodies[0],
                                              [-1.5, 0, 0.05], [0, 0, 0, 1])
    else:
  ...
  def alive_bonus(self, z, pitch):
    if self.frame % 30 == 0 and self.frame > 100 and self.on_ground_frame_counter == 0:
      ...
      position = [
          ...
          target_xyz[1] + from_dist * np.sin(angle), target_xyz[2] + 1.0
      ]
