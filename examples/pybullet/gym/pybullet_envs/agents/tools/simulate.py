...
def simulate(batch_env, algo, log=True, reset=False):
  ...
  def _define_step():
    ...
    with tf.control_dependencies([add_score, inc_length]):
      ...
      experience_summary = algo.experience(agent_indices, prevob,
                                           batch_env.action, batch_env.reward,
  ...
  def _define_summaries():
    ...
    score_summary = tf.cond(
        tf.logical_and(log, tf.cast(mean_score.count,
                                    tf.bool)), lambda: tf.summary.scalar(
                                        'mean_score', mean_score.clear()), str)
    length_summary = tf.cond(
        tf.logical_and(log, tf.cast(mean_length.count, tf.bool)), lambda: tf.
        summary.scalar('mean_length', mean_length.clear()), str)
  ...
  with tf.name_scope('simulate'):
    ...
    agent_indices = tf.cond(
        reset, lambda: tf.range(len(batch_env)), lambda: tf.cast(
