...
class PPOAlgorithm(object):
  ...
  def perform(self, agent_indices, observ):
    ...
    with tf.name_scope('perform/'):
      ...
      action = tf.cond(self._is_training,
                       output.policy.sample, lambda: output.mean)
      ...
      if self._last_state is None:
        ...
        assign_state = utility.assign_nested_vars(self._last_state,
                                                  output.state, agent_indices)
  ...
  def end_episode(self, agent_indices):
    ...
    with tf.name_scope('end_episode/'):
      return tf.cond(
          self._is_training, lambda: self._define_end_episode(agent_indices),
          str)
  ...
  def _training(self):
    ...
    with tf.name_scope('training'):
      ...
      update_summary = self._perform_update_steps(observ, action, old_mean,
                                                  old_logstd, reward, length)
      with tf.control_dependencies([update_summary]):
        penalty_summary = self._adjust_penalty(observ, old_mean, old_logstd,
                                               length)
  ...
  def _perform_update_steps(self, observ, action, old_mean, old_logstd, reward,
                            length):
    ...
    if self._config.gae_lambda:
      advantage = utility.lambda_return(
          reward, value, length, self._config.discount, self._config.gae_lambda)
    else:
    ...
    value_loss, policy_loss, summary = tf.scan(
        lambda _1, _2: self._update_step(observ, action, old_mean, old_logstd,
  ...
  def _update_step(self, observ, action, old_mean, old_logstd, reward,
    ...
    policy_loss, policy_summary = self._policy_loss(
        network.mean, network.logstd, old_mean, old_logstd, action, advantage,
        length)
  ...
  def _policy_loss(self, mean, logstd, old_mean, old_logstd, action, advantage,
                   length):
    ...
    with tf.name_scope('policy_loss'):
      ...
      with tf.control_dependencies([
          tf.cond(cutoff_count >
                  0, lambda: tf.Print(0, [cutoff_count], 'kl cutoff! '), int)
      ]):
        kl_cutoff = (self._config.kl_cutoff_coef * tf.cast(
            kl > cutoff_threshold, tf.float32) * (kl - cutoff_threshold)**2)
  ...
  def _adjust_penalty(self, observ, old_mean, old_logstd, length):
    ...
    with tf.name_scope('adjust_penalty'):
      ...
      assert_change = tf.assert_equal(
          tf.reduce_all(tf.equal(network.mean, old_mean)),
          False,
      ...
      with tf.control_dependencies([assert_change, print_penalty]):
        kl_change = tf.reduce_mean(
            self._mask(
                utility.diag_normal_kl(old_mean, old_logstd, network.mean,
                                       network.logstd), length))
        kl_change = tf.Print(kl_change, [kl_change], 'kl change: ')
        maybe_increase = tf.cond(
            kl_change > 1.3 * self._config.kl_target,
            # pylint: disable=g-long-lambda
            lambda: tf.Print(
                self._penalty.assign(self._penalty * 1.5), [0],
                'increase penalty '),
            float)
