...
NetworkOutput = collections.namedtuple('NetworkOutput',
                                       'policy, mean, logstd, value, state')


def feed_forward_gaussian(config,
                          action_size,
                          observations,
                          unused_length,
                          state=None):
  ...
  with tf.variable_scope('policy'):
    ...
    mean = tf.contrib.layers.fully_connected(
        x, action_size, tf.tanh, weights_initializer=mean_weights_initializer)
    logstd = tf.get_variable('logstd', mean.shape[2:], tf.float32,
                             logstd_initializer)
    logstd = tf.tile(
        logstd[None, None],
