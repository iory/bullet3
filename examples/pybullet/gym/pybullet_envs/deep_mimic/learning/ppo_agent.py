...
class PPOAgent(PGAgent):
  ...
  def _load_params(self, json_data):
    ...
    self.actor_stepsize_decay = 0.5 if (
        self.ACTOR_STEPSIZE_DECAY not in json_data
  ...
  def _build_nets(self, json_data):
    ...
    with tf.variable_scope('main'):
      with tf.variable_scope('actor'):
        self.a_mean_tf = self._build_net_actor(actor_net_name,
  ...
  def _build_losses(self, json_data):
    ...
    a_bound_loss = TFUtil.calc_bound_loss(self._norm_a_mean_tf,
                                          norm_a_bound_min, norm_a_bound_max)
  ...
  def _train_step(self):
    ...
    for e in range(self.epochs):
      ...
      for b in range(mini_batches):
        ...
        shuffle_actor = (actor_batch[-1] <
                         actor_batch[0]) or (actor_batch[-1] == num_exp_idx - 1)
  ...
  def _compute_batch_new_vals(self, start_idx, end_idx, val_buffer):
    ...
    if self.discount == 0:
      ...
      while curr_idx < end_idx:
        ...
        new_vals[idx0:idx1] = RLUtil.compute_return(r, self.discount,
                                                    self.td_lambda, v)
