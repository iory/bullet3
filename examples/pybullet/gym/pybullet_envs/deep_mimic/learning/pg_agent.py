...
class PGAgent(TFAgent):
  ...
  def _build_nets(self, json_data):
    ...
    actor_init_output_scale = 1 if (
        self.ACTOR_INIT_OUTPUT_SCALE_KEY not in json_data
    ...
    self.s_tf = tf.placeholder(
        tf.float32, shape=[None, s_size], name="s")  # observations
    self.tar_val_tf = tf.placeholder(
        tf.float32, shape=[None], name="tar_val")  # target value s
    self.adv_tf = tf.placeholder(
        tf.float32, shape=[None], name="adv")  # advantage
    self.a_tf = tf.placeholder(
        tf.float32, shape=[None, a_size], name="a")  # target actions
  ...
  def _calc_updated_vals(self, idx):
    ...
    if self.discount == 0:
      ...
      g_next = self.replay_buffer.get('goals',
                                      next_idx) if self.has_goal() else None
  ...
  def _calc_action_logp(self, norm_action_deltas):
    ...
    a_size = self.get_action_size()
    logp = -0.5 / (stdev * stdev) * np.sum(
        np.square(norm_action_deltas), axis=-1)
