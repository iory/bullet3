...
class ReplayBuffer(object):
  ...
  def _add_sample_buffers(self, idx):
    ...
    for key in self._sample_buffers:
      ...
      filter_idx = [
          i for i in idx
          if (self._check_flags(key, flags[i]) and not self.is_path_end(i))
      ]
  ...
  def _free_idx(self, idx):
    ...
    n = len(idx)
    if self.buffer_tail != MathUtil.INVALID_IDX:
      update_tail = idx[0] <= idx[-1] and idx[0] <= self.buffer_tail and idx[
          -1] >= self.buffer_tail
        ...
        while i < n:
          ...
          if self.is_valid_path(curr_idx):
            ...
            if start_idx < end_idx:
              ...
              self.buffers[self.PATH_START_KEY][
                  start_idx:self.buffer_size] = MathUtil.INVALID_IDX
