...
class Normalizer(object):
  ...
  def record(self, x):
    ...
    assert x.shape[-1] == size, \
        Logger.print2('Normalizer shape mismatch, expecting size {:d}, but got {:d}'.format(
            size, x.shape[-1]))
  ...
  def set_mean_std(self, mean, std):
    ...
    assert len(mean) == size and len(std) == size, \
        Logger.print2('Normalizer shape mismatch, expecting size {:d}, but got {:d} and {:d}'.format(
