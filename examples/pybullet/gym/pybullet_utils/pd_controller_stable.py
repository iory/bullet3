...
class PDControllerStableMultiDof(object):
  ...
  def computeAngVelRel(self, ornStart, ornEnd, deltaTime, bullet_client):
    ...
    q_diff = self.quatMul(
        ornStartConjugate, ornEnd
    )  # bullet_client.multiplyTransforms([0,0,0], ornStartConjugate, [0,0,0], ornEnd)
  ...
  def computePD(self, bodyUniqueId, jointIndices, desiredPositions,
                desiredVelocities, kps, kds, maxForces, timeStep):
    ...
    q1 = [
        curPos[0], curPos[1], curPos[2], curOrn[0], curOrn[1], curOrn[2],
        curOrn[3]
    ]
    # print("q1=",q1)
    ...
    qdot1 = [
        baseLinVel[0], baseLinVel[1], baseLinVel[2], baseAngVel[0],
        baseAngVel[1], baseAngVel[2], 0
    ]
    #qError = [0,0,0, 0,0,0,0]
    desiredOrn = [
        desiredPositions[3], desiredPositions[4], desiredPositions[5],
        desiredPositions[6]
    ]
    axis1 = self._pb.getAxisDifferenceQuaternion(desiredOrn, curOrn)
    # self.computeAngVel(curOrn, desiredOrn, 1, self._pb)
    angDiff = [0, 0, 0]
    qError = [
    ...
    for i in range(numJoints):
      ...
      if len(js[0]) == 4:
        desiredPos = [
            desiredPositions[qIndex], desiredPositions[qIndex + 1],
            desiredPositions[qIndex + 2], desiredPositions[qIndex + 3]
        ]
    ...
    c1 = self._pb.calculateInverseDynamics(
        bodyUniqueId, q1, qdot1, zeroAccelerations, flags=1)
    ...
    if useNumpySolver:
      ...
      dofCount = len(b)
      print(dofCount)
      qddot = self._pb.ldltSolve(
          bodyUniqueId, jointPositions=q1, b=b.tolist(), kd=kds, t=timeStep)
...
class PDControllerStable(object):
  ...
  def computePD(self, bodyUniqueId, jointIndices, desiredPositions,
    ...
    M2 = np.array(M1)
    M = (M2 + Kd * timeStep)
    c1 = self._pb.calculateInverseDynamics(bodyUniqueId, q1, qdot1,
                                           zeroAccelerations)
    c = np.array(c1)
    A = M
    b = -c + p + d
