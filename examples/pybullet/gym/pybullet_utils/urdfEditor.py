...
class UrdfEditor(object):
  ...
  def initializeFromBulletBody(self, bodyUid, physicsClientId):
    ...
    for j in range(p.getNumJoints(bodyUid, physicsClientId=physicsClientId)):
      ...
      tmpPos, tmpOrn = p.multiplyTransforms(childInertiaPos, childInertiaOrn,
                                            parentCom2JointPos,
                                            parentCom2JointOrn)
  ...
  def writeInertial(self, file, urdfInertial, precision=5):
    ...
    str = '\t\t\t<origin rpy=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\" xyz=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
        urdfInertial.origin_rpy[0],
        urdfInertial.origin_rpy[1],
        urdfInertial.origin_rpy[2],
        urdfInertial.origin_xyz[0],
        urdfInertial.origin_xyz[1],
        urdfInertial.origin_xyz[2],
        prec=precision)
    file.write(str)
  ...
  def writeVisualShape(self, file, urdfVisual, precision=5):
    ...
    file.write("\t\t<visual>\n")
    str = '\t\t\t<origin rpy="{:.{prec}f} {:.{prec}f} {:.{prec}f}" xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
        urdfVisual.origin_rpy[0],
        urdfVisual.origin_rpy[1],
        urdfVisual.origin_rpy[2],
        urdfVisual.origin_xyz[0],
        urdfVisual.origin_xyz[1],
        urdfVisual.origin_xyz[2],
        prec=precision)
    file.write(str)
    file.write("\t\t\t<geometry>\n")
    if urdfVisual.geom_type == p.GEOM_BOX:
      str = '\t\t\t\t<box size=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
          urdfVisual.geom_extents[0],
    ...
    if urdfVisual.geom_type == p.GEOM_MESH:
      ...
      str = '\t\t\t\t<mesh filename=\"{}\" scale=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
          urdfVisual.geom_meshfilename,
          urdfVisual.geom_meshscale[0],
          urdfVisual.geom_meshscale[1],
          urdfVisual.geom_meshscale[2],
          prec=precision)
      file.write(str)
    ...
    str = '\t\t\t\t<color rgba="{:.{prec}f} {:.{prec}f} {:.{prec}f} {:.{prec}f}" />\n'.format(
        ...
        urdfVisual.material_rgba[1],
        urdfVisual.material_rgba[2],
        urdfVisual.material_rgba[3],
        prec=precision)
    file.write(str)
    file.write("\t\t\t</material>\n")
    file.write("\t\t</visual>\n")
  ...
  def writeCollisionShape(self, file, urdfCollision, precision=5):
    ...
    str = '\t\t\t<origin rpy="{:.{prec}f} {:.{prec}f} {:.{prec}f}" xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
        urdfCollision.origin_rpy[0],
        urdfCollision.origin_rpy[1],
        urdfCollision.origin_rpy[2],
        urdfCollision.origin_xyz[0],
        urdfCollision.origin_xyz[1],
        urdfCollision.origin_xyz[2],
        prec=precision)
    file.write(str)
    file.write("\t\t\t<geometry>\n")
    if urdfCollision.geom_type == p.GEOM_BOX:
      str = '\t\t\t\t<box size=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
          urdfCollision.geom_extents[0],
    ...
    if urdfCollision.geom_type == p.GEOM_MESH:
      str = '\t\t\t\t<mesh filename=\"{}\" scale=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
          urdfCollision.geom_meshfilename,
          urdfCollision.geom_meshscale[0],
          urdfCollision.geom_meshscale[1],
          urdfCollision.geom_meshscale[2],
          prec=precision)
      file.write(str)
  ...
  def writeJoint(self, file, urdfJoint, precision=5):
    ...
    str = '\t\t<origin rpy=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\" xyz=\"{:.{prec}f} {:.{prec}f} {:.{prec}f}\"/>\n'.format(
        ...
        urdfJoint.joint_origin_rpy[1],
        urdfJoint.joint_origin_rpy[2],
        urdfJoint.joint_origin_xyz[0],
        urdfJoint.joint_origin_xyz[1],
        urdfJoint.joint_origin_xyz[2],
        prec=precision)
...
# def addLink(...)
  ...
  def joinUrdf(self,
    ...
    self.urdfLinks[
        childLinkIndex].urdf_inertial.origin_rpy = p.getEulerFromQuaternion(orn)
    # all visual
    for v in self.urdfLinks[childLinkIndex].urdf_visual_shapes:
      pos, orn = p.multiplyTransforms(v.origin_xyz,
    ...
    for c in self.urdfLinks[childLinkIndex].urdf_collision_shapes:
      pos, orn = p.multiplyTransforms(c.origin_xyz,
                                      p.getQuaternionFromEuler(c.origin_rpy),
                                      invJointPivotXYZInChild,
                                      invJointPivotQuatInChild)
      c.origin_xyz = pos
  ...
  def createMultiBody(self,
    ...
    obUid = p.createMultiBody(
        ...
        baseInertialFrameOrientation=p.getQuaternionFromEuler(
            base.urdf_inertial.origin_rpy),
